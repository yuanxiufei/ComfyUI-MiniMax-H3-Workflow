...
分镜拆解时遵循《全片宪法》（`剧本/00_项目配置/全局硬约束（全片宪法）.md`）的硬约束，
再叠加工业化导演方法（详见 `reference/director-industry-method.md`）：

- **分镜五要素**：每个镜头写清 主体及环境 / 构图与空间 / 光影与色调 / 摄影与成像 / 机位与角度。
- **2×2 顶视图机位（CAM1~4 + 180° 轴线）**：双人对白标准正反打 `CAM3↔CAM4`（单人特写）或
  `CAM1↔CAM2`（过肩），全程不跳轴；把机位编码写进 `angle`（如 `外反打-过肩(CAM1)`）。
- **12 节拍情绪张力曲线**：先画曲线再切时长——情绪峰值镜短切 3~5s，情绪谷值 / 空镜长镜 8~15s；
  全篇时长档位 ≥3 种（否则 QC `P1-TEMPO-FLAT` 拦截）。
- **武戏**：15s 一个完整攻防回合，拆 R1起手 / R2交击 / R3收势；每镜写 谁 + 兵器/部位 + 发力方向 + 命中结果。
- **玄幻 / 法术**：施法者 + 法器 + 能量具象 + 攻防目标 + 结果；能量具象落实到形与色；五行对应不同画面特征。
- **H3 时长预算**：整集 180s ≈ 14~15 组，每组 ≤15s，组内 3s 推进；对白镜 5~8s、动作 / 空镜 10~15s。
- **声音三层电平**：对白 0dB > 音效 −6dB > BGM −14dB；整集 −14 LUFS；H3 原生音频为主。
- **三层解耦**：`image_prompt` / `video_prompt` / audio 三套分开写，不混写、不互替。
